...
import preferences
import tkinter.messagebox, tkinter.filedialog, sys, os, os.path, gettext
...
class PrefsWindow:
    ...
    def __create_widgets(self):
        ...
        # self.prefered_piano = apply(OptionMenu, (self.main_frame, self.var_piano) + PIANO_INSTRUMENTS)  # TODO
        ...
        # self.prefered_bass = apply(OptionMenu, (self.main_frame, self.var_bass) + BASS_INSTRUMENTS)  # TODO
    ...
    def OnABC2MIDI(self):
        file = tkinter.filedialog.askopenfilename(filetypes=TUPLE_EXES)
    ...
    def OnCancel(self):
        if tkinter.messagebox.askyesno(_("Cancel"), _("Your changes will be lost.\nAre you sure?")):
            self.root.destroy()
            self = None
            return 0
...
class ChordsWindow:
    ...
    def OnDone(self):
        if self.cells[0].get() == '':
               tkinter.messagebox.showwarning(_("Empty cells"), _("You have to fill the first chord at least."))
               return 0
        else:
            chords = []
            first_time = 1
            ...
            try:
                ...
                tkinter.messagebox.showwarning(_("Error"), _("There was some error generating the song:\n") + str(e) + \
